function processData(X_train, y_train, X_test, y_test, r)
  ...
  C = 5;
  Psi_half_init = cell(C,1);
  Psi_half_init{1} = eye(d_proj); 
  Psi_half_init{2} = inv(Sigma_proj_plus);
  Psi_half_init{3} = inv(Sigma_proj_minus);
  Psi_half_init{4} = (mu_plus - mu_minus)' * Q';
  Psi_half_init{5} = ones(1,d) * Q';

  taus = cell(C,2);
  N0  = min([1000, N_plus, N_minus]);
  Pi = randperm(N0);
  I_plus  = 1:N_train; I_plus  = I_plus(logical(y_train == 1));   I_plus  = I_plus(Pi(1:N0));
  Pi = randperm(N0);
  I_minus = 1:N_train; I_minus = I_minus(logical(y_train == -1)); I_minus = I_minus(Pi(1:N0));
  clear Pi;
  disp('Computing taus...');
  tic;
  for i=1:C
    ...
    for j=1:N0
      ...
      taus{i,1}(j) = norm(Psi_half_init{i} * (X_proj(:,I_plus(j)) - mu_proj_plus), 2);
      taus{i,2}(j) = norm(Psi_half_init{i} * (X_proj(:,I_minus(j)) - mu_proj_minus), 2);
    end
  end
  ...
  Psi_half = [];
  for i=1:C
    Psi_half = [Psi_half; Psi_half_init{i} / thresholds(i)];
  end
  ...
  for j=1:N0
    ...
    tau_agg(j,1) = norm(Psi_half * (X_proj(:, I_plus(j)) - mu_proj_plus), 2);
    tau_agg(j,2) = norm(Psi_half * (X_proj(:, I_minus(j)) - mu_proj_minus), 2);
  end
  toc;
  fprintf('tau_agg 0.75 quantiles:\n\tplus: %.4f\n\tminus: %.4f\n', quantile(tau_agg(:,1), 0.75), quantile(tau_agg(:,2), 0.75));
  save(sprintf('process_data_results_r%d.mat', r));
end
